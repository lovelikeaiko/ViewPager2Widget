...
//merged by HP stared
import com.android.internal.widget.ViewPager.OnPageItemClickListener;
...
public class RemoteViews implements Parcelable, Filter {
	...
	//merge by HP started
    private static final Object[] sMultiParamMethodsLock = new Object[0];
    private static final ArrayMap<Class<? extends View>, ArrayMap<MutablePair<String, Class[]>, Method>> sMultiParamMethods =
            new ArrayMap<Class<? extends View>, ArrayMap<MutablePair<String, Class[]>, Method>>();
	//merge by HP ended
     ...
     * This pair is used to perform lookups in sMethods without causing allocations.
     */
    private final MutablePair<String, Class[]> mMultiParamPair =
            new MutablePair<String, Class[]>(null, null);			
	//merge by HP ended
    ...
    private class SetEmptyView extends Action {
        ...
        public void apply(View root, ViewGroup rootParent, OnClickHandler handler) {
            ...
            //Log.i(LOG_TAG, "RemoteViews.SetEmptyView, apply, view" + view);

            //modified by HP started    
            if (!(view instanceof AdapterView<?>) && !(view instanceof ViewPager)) return;
            ...
            if (view instanceof AdapterView<?>){
                ...
                //Log.i(LOG_TAG, "RemoteViews.SetEmptyView, mEmptyView" + emptyView);

                
                if (emptyView == null) return; 

                vp.setEmptyView(emptyView);
            }
    ...
    private class SetOnClickFillInIntent extends Action {
        ...
        public void apply(View root, ViewGroup rootParent, final OnClickHandler handler) {
            ...
            if (target == root) {
                ...
                //Log.i(LOG_TAG, "SetOnClickFillInIntent.setTagInternal,fillInIntent=" + fillInIntent
                //    + ", target=" + target);
                //merged by HP ended                  
                ...
                OnClickListener listener = new OnClickListener() {
                    public void onClick(View v) {
                        ...
                        //merged by HP started
                        //Log.i(LOG_TAG, "SetOnClickFillInIntent.onClick, handler=" + handler
                        //    + ", v=" + v);
                        //merged by HP ended                        
                    }
    ...
    private class SetPendingIntentTemplate extends Action {
        ...
        public void apply(View root, ViewGroup rootParent, final OnClickHandler handler) {
			...
			//merged by HP started
			//android.util.Log.d(LOG_TAG, "root=" + root + ", rootParent=" + rootParent + ", target=" + target 
			//	+ ", viewId=" + viewId);
			//merged by HP ended
                ...
                OnItemClickListener listener = new OnItemClickListener() {
                    public void onItemClick(AdapterView<?> parent, View view,
                        ...
                        if (view instanceof ViewGroup) {
                            ...
                            //merged by HP started
                            //Log.i(LOG_TAG, "SetPendingIntentTemplate.onItemClick, vg=" + vg
                            ...
                            //merged by HP started
                            //Log.i(LOG_TAG, "SetPendingIntentTemplate.onItemClick, handler=" + handler
                            //    + ", view=" + view);
                            //merged by HP ended
            ...
            //merged by HP started
                ...
                ViewPager vp = (ViewPager) target;
                OnPageItemClickListener listener = new OnPageItemClickListener() {
                    public void OnPageItemClick(View viewpager, View curPage) {

                        //Log.i(LOG_TAG, "SetPendingIntentTemplate.ViewPager,in onClick, viewpager=" + viewpager);                        
                        
                        // The view should be a frame layout
                        if (viewpager instanceof ViewGroup) {
                            ViewGroup vg = (ViewGroup) viewpager;

                            if(curPage instanceof ViewGroup){
                                vg = (ViewGroup) curPage;
                            } else {
                                return;
                            }
                            ...
                            //Log.i(LOG_TAG, "SetPendingIntentTemplate.ViewPager,in onClick, fillInIntent="
                            //    + fillInIntent);  
                            
                            if (fillInIntent == null) return;

                            final Rect rect = getSourceBounds(viewpager);
                         

                            final Intent intent = new Intent();
                            intent.setSourceBounds(rect);
                            handler.onClickHandler(viewpager, pendingIntentTemplate, fillInIntent);  
                            //merged by HP started
                            //Log.i(LOG_TAG, "SetPendingIntentTemplate.ViewPager.onClick, handler=" + handler
                            //    + ", view=" + viewpager);
                            //merged by HP ended     
                ...
                vp.setOnPageItemClickListener(listener);
                vp.setTag(pendingIntentTemplate);

                //Log.i(LOG_TAG, "SetPendingIntentTemplate.ViewPager.onClick, after setOnClickListener"
                //    + ", vp=" + vp + ", listener" + listener);
	...
	//merged by HP started
    private Method getMultiParamMethod(View view, String methodName, Class[] paramTypes) {
        Method method;
        Class<? extends View> klass = view.getClass();

        synchronized (sMultiParamMethodsLock) {
            ArrayMap<MutablePair<String, Class[]>, Method> methods = sMultiParamMethods.get(klass);
            if (methods == null) {
                methods = new ArrayMap<MutablePair<String, Class[]>, Method>();
                sMultiParamMethods.put(klass, methods);
            }

            mMultiParamPair.first = methodName;
            mMultiParamPair.second = paramTypes;

            method = methods.get(mMultiParamPair);
            if (method == null) {
                try {
                    if (paramTypes == null) {
                        method = klass.getMethod(methodName);
                    ...
                    throw new ActionException("view: " + klass.getName() + " doesn't have method: "
                            + methodName + getParameters(paramTypes));
                }
                ...
                methods.put(new MutablePair<String, Class[]>(methodName, paramTypes), method);
            }
        }
    ...
    private static String getParameters(Class[] paramTypes) {
    ...
    private class ViewPaddingAction extends Action {
     ...
     //merged by HP started
	public void setAppWidgetIDForViewPager(int viewId, int appWidgetID){
		setInt(viewId, "setAppWidgetID", appWidgetID);
	}

	
    private class ViewPagerSetCurrentItemAction extends Action {
        public ViewPagerSetCurrentItemAction(int viewId, int item, boolean smoothScroll) {
			this.viewId = viewId;
            this.item = item;
            this.smoothScroll = smoothScroll;
        }

        public ViewPagerSetCurrentItemAction(Parcel parcel) {
			viewId = parcel.readInt();
            item = parcel.readInt();
            smoothScroll = (parcel.readInt()!=0);
        }
        ...
        public void writeToParcel(Parcel dest, int flags) {
			...
			dest.writeInt(viewId);
            dest.writeInt(item);
			dest.writeInt(smoothScroll?1:0);	
        }
        ...
        public void apply(View root, ViewGroup rootParent, OnClickHandler handler) {
            ...
            if (target == null) return;
			if (!(target instanceof ViewPager)) return;
            ((ViewPager)target).setCurrentItem(item, smoothScroll);
        }

        public String getActionName() {
            return "ViewPagerSetCurrentItemAction";
        }

        int item;
		boolean smoothScroll;

        public final static int TAG = 21;
    }	
	
	public void setViewPagerCurrentItem(int viewId, int item, boolean smoothScroll){
		addAction(new ViewPagerSetCurrentItemAction(viewId, item, smoothScroll));
	}
	
	//merged by HP ended

	//merged by HBF started
	
    /**
     * Equivalent to calling
     * {@link com.android.internal.widget.ViewPager#setOnPageChangeListener(com.android.internal.widget.ViewPager.OnPageChangeListener)}
     * to launch the provided {@link PendingIntent}.
     */
	public void setOnPageChangedPendingIntent(int viewId, PendingIntent pendingIntent) {
        addAction(new SetOnPageChangedPendingIntent(viewId, pendingIntent));
    }
	
    private class SetOnPageChangedPendingIntent extends Action {
        public SetOnPageChangedPendingIntent(int id, PendingIntent pendingIntent) {
            this.viewId = id;
            this.pendingIntent = pendingIntent;
        }

        public SetOnPageChangedPendingIntent(Parcel parcel) {
            viewId = parcel.readInt();
        ...
        public void apply(View root, ViewGroup rootParent, final OnClickHandler handler) {
			...
			if (!(target instanceof ViewPager)) return;
            // If the pendingIntent is null, we clear the onClickListener
            OnPageChangeListener listener = null;
            if (pendingIntent != null) {
                listener = new OnPageChangeListener() {
					...
					public void onPageSelected(int position) {
						// Find target view location in screen coordinates and
                        // fill into PendingIntent before sending.
                        final Rect rect = getSourceBounds(target);

                        final Intent intent = new Intent();
                        intent.setSourceBounds(rect);
						intent.putExtra("position", position);
                        handler.onClickHandler(target, pendingIntent, intent);
					}
        ...
        public String getActionName() {
            return "SetOnPageChangedPendingIntent";
        }
                ...
                for (int i=0; i<count; i++) {
                    ...
                    switch (tag) {
                        ...
                        case TextViewDrawableColorFilterAction.TAG:
                            ...
                            break;
						//merged by HP started
                        case ViewPagerSetCurrentItemAction.TAG:
                            mActions.add(new ViewPagerSetCurrentItemAction(parcel));
                            break;
						//merged by HP ended

						//merged by HBF started
                        case SetOnPageChangedPendingIntent.TAG:
                            mActions.add(new SetOnPageChangedPendingIntent(parcel));
                            break;
						//merged by HBF ended
    ...
    private void addAction(Action a) {
        ...
        //merged by HP stared
        //Log.i(LOG_TAG, "RemoteViews.addAction(), a=" + a); 
        //merged by HP ended         
    ...
    private void performApply(View v, ViewGroup parent, OnClickHandler handler) {
        if (mActions != null) {
            ...
            for (int i = 0; i < count; i++) {
                //merged by HP stared
                //Log.i(LOG_TAG, "RemoteViews.performApply(), i=" + i + ", a=" + mActions.get(i)); 
                //merged by HP ended                      
                Action a = mActions.get(i);
